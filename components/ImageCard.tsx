...
export const ImageCard: React.FC<ImageCardProps> = ({ image, onImageClick }) => {
    ...
    if (!image || image.loading) {
        return (
            <div className="relative group bg-slate-100 rounded-lg shadow-md aspect-square flex flex-col items-center justify-center p-4 border border-slate-200 overflow-hidden text-slate-600">
    ...
    return (
        <div className="relative group bg-slate-100 rounded-lg shadow-md aspect-square flex items-center justify-center p-4 border border-slate-200 overflow-hidden">
            {image.base64 ? (
                <img src={image.base64} alt={image.prompt} onClick={() => onImageClick(image.base64!)} className="absolute inset-0 w-full h-full object-cover rounded-lg cursor-pointer"/>
            ) : (
            ...
            <div className="absolute top-3 right-3 flex flex-col gap-2 transition-opacity opacity-0 group-hover:opacity-100">
                ...
                <button onClick={handleCopyPrompt} disabled={!image.base64} className="p-2 rounded-full bg-slate-50 hover:bg-slate-200 text-slate-800 shadow-lg transition-colors disabled:bg-slate-400 active:scale-95" title="Copy Prompt">
